...
# CẤU HÌNH
# ══════════════════════════════
TOKEN      = "8671815898:AAFce-Ld9--Eyn5oNE27zQ5ql5vDNhpzVfI"
ADMIN_ID   = 7510588294
SHOP_NAME  = "Vantu Shop"
ADMIN_USER = "khoga36"

# Cấu hình nạp thẻ (dùng API TrumThe hoặc CardPro)
# Để trống nếu chưa có API
THE_API_KEY = ""  # API key nạp thẻ
THE_API_URL = "https://trumthe.vn/chargingws/v2"
...
def kb_wallet():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💵 Nạp Thẻ Cào",      callback_data="napthe")],
...
"""
    ...
    # ── XÚC XẮC ──
        ...
        if total >= 10:
            prize = cost * 3
            result = f"🎉 *THẮNG!* Nhận *{prize:,}đ*"
        elif total >= 7:
            prize = cost
            result = f"😊 Hoà! Nhận lại *{prize:,}đ*"
        else:
            prize = 0
            result = "😢 Thua rồi!"
        u["balance"] = u["balance"] - cost + prize
